...
def seleccionar_plantilla(config):
    ...
    if not archivos:
        ...
        pausar()
        return None
    opciones = [(f.name, str(f)) for f in archivos]
...
def seleccionar_mes(config, carga_horaria):
    ...
    opciones = [(sh, sh) for sh in xl.sheet_names]
    mes = inquirer.select(
...
def main():
    ...
    while True:
        ...
        op = inquirer.select(
            ...
            choices=[
                ("Exportar cursos", "1"),
                ("Cambiar archivo de carga horaria", "2"),
                ("Cambiar mes/sheet", "3"),
                ("Ingresar feriados", "4"),
                ("Cambiar plantilla", "5"),
                ("Mostrar carga horaria y cursos", "6"),
                ("Mostrar configuración actual", "7"),
                ("Salir", "0"),
            ],
            default="1",
        ).execute()
